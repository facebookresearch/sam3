# Copyright (c) Aishwarya Kamath & Nicolas Carion. Licensed under the Apache
...
# def _torch_compile_if_supported(func):
#     # Triton only supports devices of CUDA Capability >= 7.0, So Quadro GP100
#     # on the FAIR cluster is too old to be supported by the Triton GPU compiler.
#     # But we need to support it for the local demo backend on the FAIR cluster.
#     try:
...
#     logging.warning(
#         f"skip torch.compile due to too old cuda_capability: {cuda_capability}"
#     )
#     return func


# We turn off torch.compile since on real inputs with varying size per batch,
# it gives almost no speedup and sometimes a slowdown due to compiling and
# tracing different input sizes (even with `dynamic=True` in torch.compile).
# @_torch_compile_if_supported
...
def masks_to_boxes(masks):
    ...
    mask_has_fg_pixels = masks.flatten(-2).any(-1)
    boxes = boxes * mask_has_fg_pixels.reshape(*boxes.shape[:-1], 1)
    return boxes
