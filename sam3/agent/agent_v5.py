...
mllm_system_prompt_path = "system_prompt_v5.txt"
...
error_save_dir = f"{DEFAULT_USER_FOLDER}/code/out/none_out"
debug_save_dir = f"{DEFAULT_USER_FOLDER}/code/out/agent_debug_out"

THRESHOLD = 0.5  # Default threshold for SAM3 segmentation

NUM_RETRIES = 10  # Number of retries for MLLM calls


def agent_inference(img_path: str, initial_text_prompt: str, debug: bool = False, send_generate_request=send_generate_request, call_sam_service=call_sam_service, max_generations: int = 100):
    """ 
    ...
    with open(mask_selection_system_prompt_path, "r") as f:
        mask_selection_system_prompt = f.read().strip()
    ...
    messages = [
        ...
        { "role": "user", "content": [
            ...
            {"type": "text", "text": f"The above image is the raw input image. The initial user input query is: '{initial_text_prompt}'."},
        ]},
    ]
    print("\nInitial text prompt:\n\n", initial_text_prompt)
    print("\n\nInitial image path:\n\n", img_path)
    ...
    def _prune_messages_for_next_round(messages_list, used_text_prompts, latest_sam3_text_prompt):
        ...
        for idx in range(len(messages_list) - 1, 1, -1):
            ...
            for content in msg["content"]:
                if isinstance(content, dict) and content.get("type") == "text" and "<tool>" in content.get("text", "") and "segment_phrase" in content.get("text", ""):
                    part2_start_idx = idx
                    break
    ...
    # Replace the second message so it always contains exactly two content items (image + text with appended warning)
        ...
        if part2 and len(previously_used) > 0:  # only add a warning if part2 exists and there are previously used prompts
            warning_text = (
                f"Note that we have previously called the segment_phrase tool with each \"text_prompt\" in this list: {list(previously_used)}, but none of the generated results were satisfactory. So make sure that you do not use any of these phrases as the \"text_prompt\" to call the segment_phrase tool again."
            )
            ...
            part1[1] = {
                ...
                "content": [
                    {"type": "image", "image": img_path},
                    {"type": "text", "text": f"The above image is the raw input image. The initial user input query is: '{initial_text_prompt}'." + " " + warning_text},
                ],
            }
            assert len(part1[1]["content"]) == 2, "Second message must contain exactly 2 content items after pruning."
    ...
    while generated_text is not None:
        ...
        tool = generated_text.split("<tool>")[-1].split("</tool>")[0].strip().replace(r"}}}", r"}}")    #sometimes the MLLM generates an extra } at the end, so we need to remove it, TODO: check why this happens
        try:
        ...
        if PATH_TO_LATEST_OUTPUT_JSON == "":
            assert tool_call["name"] == "segment_phrase" or tool_call["name"] == "report_no_mask" # "The first tool call must be segment_phrase or report_no_mask"
        ...
        if tool_call["name"] == "segment_phrase":
            ...
            if current_text_prompt in USED_TEXT_PROMPTS:
                ...
                if num_masks == 0:
                    ...
                    messages.append({"role": "user", "content": [{"type": "text", "text": sam3_output_text_message}, {"type": "image", "image": sam3_output_image_path}]})
                print("\n\nsam3_output_text_message:\n", sam3_output_text_message)
            ...
            for i in range(num_masks):  
                ...
                iterative_checking_messages = [
                    ...
                    { "role": "user", "content": [
                        ...
                        {"type": "text", "text": f"Image with the predicted segmentation mask rendered on it: "},
                        {"type": "image", "image": image_w_mask_i_path},
                        {"type": "text", "text": f"Image with the zoomed-in mask: "},
            ...
            # # simply ignore the masks generated by the initial call to "select_masks_and_return", and rely on our updated call to "select_masks_and_return"
            # # look into the message history to find the latest output image path
            # if len(current_outputs["pred_masks"]) > 1:
            #     latest_output_image_path = messages[-1]["content"][-1]["image"]
            #     mask_selection_messages = [
            #             { "role": "system", "content": mask_selection_system_prompt}, 
            #             { "role": "user", "content": [
            #                 {"type": "text", "text": f"The raw input image: "},
            #                 {"type": "image", "image": img_path}, 
            #                 {"type": "text", "text": f"The initial user input query is: '{initial_text_prompt}'"},
            #                 {"type": "text", "text": f"Image with all available masks rendered on it: "},
            #                 {"type": "image", "image": latest_output_image_path},
            #                 {"type": "text", "text": f"There are a total of {len(current_outputs['pred_masks'])} available masks."},
            #             ]},
            #         ]
            #     mask_selection_generated_text = send_generate_request(mask_selection_messages)
                
                
            #     print("\n\n Generated_text:\n\n", mask_selection_generated_text)
            #     assert ("<tool>" in mask_selection_generated_text)
            #     mask_selection_generated_text = mask_selection_generated_text.split("</tool>", 1)[0] + "</tool>"
            #     tool = mask_selection_generated_text.split("<tool>")[-1].split("</tool>")[0].strip().replace(r"}}}", r"}}")    #sometimes the MLLM generates an extra } at the end, so we need to remove it, TODO: check why this happens
            #     try:
            #         tool_call = json.loads(tool)
            #     except json.JSONDecodeError:
            #         raise ValueError(f"Invalid JSON in tool call: {tool}")
            #     assert tool_call["name"] == "select_masks_and_return"
            # else:
            #     pass
            # # ======================================== Inference Time Scaling ==================================================
        ...
        messages = _prune_messages_for_next_round(messages, USED_TEXT_PROMPTS, LATEST_SAM3_TEXT_PROMPT)
    ...
    raise ValueError(rf"Generated text is None, which is unexpected. Please check the Qwen server and the input parameters for image path: {img_path} and initial text prompt: {initial_text_prompt}.")
