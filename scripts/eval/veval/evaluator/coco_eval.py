...
from .utils.dist import all_gather, gather_to_rank_0_via_filesys
...
class CocoEvaluator:
    def __init__(
        ...
        self.use_self_evaluate = use_self_evaluate
    ...
    def synchronize_between_processes(self):
        ...
        if self.dump is not None:
            dumped_file = (
                Path(self.dump_dir) / f"coco_predictions_{dist.get_rank()}.json"
            )
            ...
            # else:
            #     dump = dist.all_gather(self.dump, force_cpu=True)
            # self.dump = sum(dump, [])
...
def create_common_coco_eval(
    ...
    if metrics_dump_dir is not None:
        dumped_file = (
            Path(metrics_dump_dir) / f"coco_eval_img_metrics_{dist.get_rank()}.json"
        )
        logging.info(f"COCO evaluator: Dumping local predictions to {dumped_file}")
        with g_pathmgr.open(str(dumped_file), "w") as f:
