...
void SortInstancesByIgnore(
  ...
  std::stable_sort(
      ...
      [&ignores](size_t j1, size_t j2) {
        return static_cast<int>((*ignores)[j1]) <
            static_cast<int>((*ignores)[j2]);
      });
}
...
std::vector<ImageEvaluation> EvaluateImages(
  ...
  for (auto i = 0; i < num_images; ++i) {
    for (auto c = 0; c < num_categories; ++c) {
      ...
      if (static_cast<int>(detection_sorted_indices.size()) > max_detections) {
...
int BuildSortedDetectionList(
  ...
  for (auto i = 0; i < num_images; ++i) {
    ...
    for (int d = 0; d < static_cast<int>(evaluation.detection_scores.size()) &&
         d < max_detections;
...
py::dict Accumulate(
  ...
  if (num_area_ranges * num_images * num_categories != evaluations.size()) {
    std::cout
        << "ERROR: The number of provided evaluations is incorrect. Expected:"
        << std::endl;
    std::cout << "\t num_categories=" << num_categories << std::endl;
    std::cout << "\t num_area_ranges=" << num_area_ranges << std::endl;
    std::cout << "\t num_images=" << num_images << std::endl;
    std::cout << "\t total=" << num_area_ranges * num_images * num_categories
              << std::endl;
    std::cout << "Instead, we got size=" << evaluations.size() << std::endl;
    throw std::runtime_error("Invalid evaluation tensor!");
  }
