...
class LVVISeval:
    ...
    def summarize(self):
        ...
        def _summarize(ap=1, iouThr=None, areaRng="all", maxDets=100):
            ...
            print(
                iStr.format(titleStr, typeStr + "b", iouStr, areaRng, maxDets, mean_bs)
            )
            print(
                iStr.format(titleStr, typeStr + "n", iouStr, areaRng, maxDets, mean_ns)
            )
            return mean_s, mean_bs, mean_ns

        def _summarizeDets():
            stats = np.zeros((36,))
            stats[0:3] = _summarize(1)
            stats[3:6] = _summarize(1, iouThr=0.5, maxDets=self.params.maxDets[2])
            stats[6:9] = _summarize(1, iouThr=0.75, maxDets=self.params.maxDets[2])
            stats[9:12] = _summarize(1, areaRng="small", maxDets=self.params.maxDets[2])
            stats[12:15] = _summarize(
                1, areaRng="medium", maxDets=self.params.maxDets[2]
            )
            stats[15:18] = _summarize(
                1, areaRng="large", maxDets=self.params.maxDets[2]
            )
            stats[18:21] = _summarize(0, maxDets=self.params.maxDets[0])
            stats[21:24] = _summarize(0, maxDets=self.params.maxDets[1])
            stats[24:27] = _summarize(0, maxDets=self.params.maxDets[2])
            stats[27:30] = _summarize(
                0, areaRng="small", maxDets=self.params.maxDets[2]
            )
            stats[30:33] = _summarize(
                0, areaRng="medium", maxDets=self.params.maxDets[2]
            )
            stats[33:36] = _summarize(
        ...
        def _summarizeKps():
            stats = np.zeros((30,))
            stats[0:3] = _summarize(1, maxDets=20)
            stats[3:6] = _summarize(1, maxDets=20, iouThr=0.5)
            stats[6:9] = _summarize(1, maxDets=20, iouThr=0.75)
            stats[9:12] = _summarize(1, maxDets=20, areaRng="medium")
            stats[12:15] = _summarize(1, maxDets=20, areaRng="large")
            stats[15:18] = _summarize(0, maxDets=20)
            stats[18:21] = _summarize(0, maxDets=20, iouThr=0.5)
            stats[21:24] = _summarize(0, maxDets=20, iouThr=0.75)
            stats[24:27] = _summarize(0, maxDets=20, areaRng="medium")
            stats[27:30] = _summarize(0, maxDets=20, areaRng="large")
            return stats
