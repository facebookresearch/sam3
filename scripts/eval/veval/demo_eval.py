...
import onevision.utils.dist as dist
import pycocotools.mask as maskUtils
from iopath.common.file_io import g_pathmgr
from onevision.data.datasets.coco_eval import CocoEvaluator, RARITY_BUCKETS

from onevision.utils.masks_ops import compute_F_measure
...
class DemoEval(COCOeval):
    ...
    def accumulate(self, p=None):
        ...
        TPs = np.zeros((len(p.iouThrs),), dtype=np.int64)
        FPs = np.zeros((len(p.iouThrs),), dtype=np.int64)
        pgFPs = np.zeros((len(p.iouThrs),), dtype=np.int64)
        ...
        # compute precision recall and F1
        precision = TPs / (TPs + FPs + 1e-4)
        pgprecision = TPs / (TPs + pgFPs + 1e-4)
        assert np.all(precision <= 1)
        recall = TPs / (TPs + FNs + 1e-4)
        assert np.all(recall <= 1)
        F1 = 2 * precision * recall / (precision + recall + 1e-4)
        pgF1 = 2 * pgprecision * recall / (pgprecision + recall + 1e-4)
        ...
        self.eval = {
            ...
            "TPs": TPs,
            "FPs": FPs,
            "positive_global_FPs": pgFPs,
            "FNs": FNs,
            "precision": precision,
            "positive_global_precision": pgprecision,
            "recall": recall,
            "F1": F1,
            "positive_global_F1": pgF1,
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            ...
            stats[29] = _summarize(metric="positive_global_precision")
            stats[30] = _summarize(metric="positive_global_F1")
            stats[31] = _summarize(iouThr=0.5, metric="positive_global_precision")
            stats[32] = _summarize(iouThr=0.5, metric="positive_global_F1")
            stats[33] = _summarize(iouThr=0.75, metric="positive_global_precision")
            stats[34] = _summarize(iouThr=0.75, metric="positive_global_F1")
            return stats
...
DEMO_METRICS = [
    ...
    "F",
    "J&F",
    "positive_global_Precision",
    "positive_global_F1",
    "positive_global_Precision@0.5",
    "positive_global_F1@0.5",
    "positive_global_Precision@0.75",
    "positive_global_F1@0.75",
]
